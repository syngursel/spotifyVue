		...
		<div class="content bg-image bg-gray-darker">
			<div class="push-50-t push-15 clearfix">
				...
				<div class="push-15-r pull-left animated fadeIn">
					<img v-if='user.images !== undefined' class="img-avatar img-avatar96 img-avatar-thumb" :src="user.images[0].url">
				</div>
        ...
        var that = this;
				...
				for (var i = 0; i < this.playlists.items.length; i++) {
					...
					.then(function(res) {
						count++;
						if(res.error === undefined){
              that.tracks = res.items;
							let trackNumbers = [];
							while(true){
								let trackNumber = Math.floor(Math.random() * that.tracks.length);
								if(trackNumbers.includes(trackNumber)){
									continue;
								} else {
									if(!that.playlistTracks.includes(that.tracks[trackNumber].track.uri)){
										if(numberOfTracks === trackNumbers.length){
											break;
										} else {
                      that.playlistTracks.push(that.tracks[trackNumber].track.uri);
											trackNumbers.push(trackNumber);
										}
									} else continue;
								}
							}
							if(count >= that.playlists.items.length){
                that.postTracksToPlaylist(playlistId);
							}
						} else {
							//App.loader("hide");
							//swal('Oops...', 'Something went wrong!', 'error');
